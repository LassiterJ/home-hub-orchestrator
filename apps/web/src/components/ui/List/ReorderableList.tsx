...
export function List<T>(props: ListProps<T>) {
   ...
   const getItemById = (id: string): T | undefined => currentItems.find((i) => i.id === id)
   const getIndexById = (id: string): number => currentItems.findIndex((i) => i.id === id)
...
function ListHandle({ asChild, children }: { asChild?: boolean; children: ReactNode }) {
   ...
   // Stop React Flow canvas/node dragging from intercepting handle interactions
   const stopFlowPointerPropagation = (e: any) => {
      if (typeof e.stopPropagation === 'function') e.stopPropagation()
   }

   const compose = (childHandler?: (e: any) => void) => (e: any) => {
      stopFlowPointerPropagation(e)
      if (typeof childHandler === 'function') childHandler(e)
   }
   ...
   if (asChild) {
      ...
      return cloneElement(only as any, {
         ref: refCb,
         onPointerDownCapture: compose((only as any).props?.onPointerDownCapture),
         onMouseDownCapture: compose((only as any).props?.onMouseDownCapture),
         onTouchStartCapture: compose((only as any).props?.onTouchStartCapture),
      })
   }
   ...
   return (
      <div
         ref={refCb}
         onPointerDownCapture={stopFlowPointerPropagation}
         onMouseDownCapture={stopFlowPointerPropagation}
         onTouchStartCapture={stopFlowPointerPropagation}
      >
         {children}
      </div>
   )
