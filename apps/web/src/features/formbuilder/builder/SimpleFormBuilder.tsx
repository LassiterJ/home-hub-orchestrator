...
/**
 * Props for the SimpleFormBuilder component
 */
...
}> = ({ fields, selectedFieldId, onSelectField, onRemoveField }) => {
  return (
    <div className="space-y-2">
...
/**
 * Main SimpleFormBuilder component
 */
export const SimpleFormBuilder: React.FC<SimpleFormBuilderProps> = ({
  ...
  // Notify parent of schema changes (debounced)
  React.useEffect(() => {
    onSchemaChange?.(debouncedSchema);
  }, [debouncedSchema, onSchemaChange]);

  // Generate TSX code
  const tsxCode = useMemo(() => {
    return generateFormTsx(schema);
  }, [schema]);
  ...
  // selectedField is now provided by the hook
        ...
        <Tabs defaultValue="preview" className="h-full">
          ...
          <TabsContent value="preview" className="h-full p-4">
            <div className="h-full overflow-auto">
              <FormRenderer schema={schema} debug={debug} />
            </div>
          </TabsContent>
